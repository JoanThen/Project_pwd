...
<body class="min-h-screen font-sans text-gray-100 bg-gradient-to-br from-blue-900 via-gray-900 to-black">
    ...
    <main class="max-w-7xl mx-auto px-6 py-10">
        ...
        <div class="overflow-x-auto bg-gray-800 shadow-md rounded-lg">
            <table class="w-full text-sm text-left text-gray-300" id="laporanPengembalianTable">
                ...
                <div class="space-y-4">
                    @foreach($pengembalians->take(5) as $recent)
                        <div class="flex items-center p-3 bg-gray-700/50 rounded-lg">
                            ...
                            <div class="flex-1">
                                <p class="text-white font-medium">{{ $recent->peminjaman->barang->nama ?? 'Tidak diketahui' }}
                                </p>
                                <p class="text-gray-400 text-sm">Dikembalikan oleh
                                    {{ $recent->peminjaman->nama_peminjam ?? 'Tidak diketahui' }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-blue-400 text-sm">
                                    {{ \Carbon\Carbon::parse($recent->tanggal_pengembalian)->format('d M Y') }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
    ...
    const table = document.getElementById("LaporanPengembalianTable");
